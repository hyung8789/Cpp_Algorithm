...
static size_t compareCount[TOTAL_SORT_FUNC_COUNT] = { 0, }; //비교 횟수 카운트

struct TRACE_RESULT
{
	TRACE_RESULT();

	std::chrono::nanoseconds accDuration; //누적 소요 시간
	std::chrono::nanoseconds minDuration; //최소 소요 시간
	std::chrono::nanoseconds maxDuration; //최대 소요 시간
...
void IncreaseCompareCount(const char*);
void DispCompareCount(const char*);
size_t GetCompareCount(const char*);
