...
///이하 수정
		...
		-		B		B		BR
					...
					TODO : 트리의 최상위 루트 노드가 빨간색이 될 경우 검은색으로 변경 시킬 것, 
					최상위 루트 노드가 이중 검은색 노드가 되어도 단일 검은색 노드로 변경 시킬 경우
					양 방향의 검은 노드의 수는 변함없으므로 단일 검은색으로 변경시킬 것
...
//ok
		...
		(이중 검은색 노드 (이동 대상 노드)의 부모 노드의 오른쪽 자식 == 이중 검은색 노드 (이동 대상 노드))
			...
			이중 검은색 노드 (이동 대상 노드)의 형제 노드로 자신의 한 검은색을 전달 할 경우,
				
			- 이중 검은색 노드 (이동 대상 노드)의 부모 노드 기준 왼쪽 경로의 검은 노드의 수 증가
			- 이중 검은색 노드 (이동 대상 노드)의 부모 노드 기준 오른쪽 경로의 검은 노드의 수 감소

							B
				B (sibling)			B (moveTarget)
			B				

			=> 이중 검은색 노드 (이동 대상 노드)의 부모 노드 기준 왼쪽 경로의 검은 노드의 수 감소를 통한 복구 및
			이중 검은색 노드 (이동 대상 노드)의 부모 노드 기준 오른쪽 경로의 검은 노드의 수 증가를 통한 복구를 위해
			이중 검은색 노드 (이동 대상 노드)의 부모 노드 기준 우회전
				
			- 이중 검은색 노드 (이동 대상 노드)의 형제 노드 (회전 발생 전) 기준 왼쪽 경로의 검은 노드의 수 감소에 따른 복구
			- 이중 검은색 노드 (이동 대상 노드)의 형제 노드 (회전 발생 전) 기준 오른쪽 경로의 검은 노드의 수 증가에 따른 복구
...
//ok
		...
		(이중 검은색 노드 (이동 대상 노드)의 부모 노드의 오른쪽 자식 == 이중 검은색 노드 (이동 대상 노드))
			...
			=> 이중 검은색 노드 (이동 대상 노드)의 부모 노드의 색에 따라,
				...
				=> DEF1)을 위반하는 이중 검은색 노드 (이동 대상 노드)의 한 검은색을 없애기 위해,
				이중 검은색 노드 (이동 대상 노드)의 부모 노드로 자신의 한 검은색을 전달 할 경우,

				- 이중 검은색 노드 (이동 대상 노드)의 부모 노드 기준 왼쪽 경로의 검은 노드의 수 증가
				- 이중 검은색 노드 (이동 대상 노드)의 부모 노드 기준 오른쪽 경로의 검은 노드의 수 변동없음

								B
					B (sibling)			B (moveTarget)
				B		B

				=> 이중 검은색 노드 (이동 대상 노드)의 부모 노드 기준 왼쪽 경로의 검은 노드의 수 감소에 따른 복구를 위해,
				이중 검은색 노드 (이동 대상 노드)의 형제 노드의 색 변경 (검은색 => 빨간색)

				- 이중 검은색 노드 (이동 대상 노드)의 부모 노드 기준 왼쪽 경로의 검은 노드의 수 감소에 따른 복구
...
//ok
		...
		(이중 검은색 노드 (이동 대상 노드)의 부모 노드의 오른쪽 자식 == 이중 검은색 노드 (이동 대상 노드))
			...
			=> 중복 최적화를 위해 문제의 형태를 -BB BR로 바꿔야 함
			먼저, 이중 검은색 노드 (이동 대상 노드)의 형제 노드와 이중 검은색 노드 (이동 대상 노드)의 형제 노드의 빨간색 자식 노드 간 색 SWAP (검은색 => 빨간색, 빨간색 => 검은색)

			- 이중 검은색 노드 (이동 대상 노드)의 형제 노드 기준 왼쪽 경로의 검은 노드의 수 변동없음
			- 이중 검은색 노드 (이동 대상 노드)의 형제 노드 기준 오른쪽 경로의 검은 노드의 수 감소

							B (or R)
				R (sibling)			DB (moveTarget)
			B		B

			=> 이중 검은색 노드 (이동 대상 노드)의 형제 노드 기준 오른쪽 경로의 검은 노드의 수 증가를 통한 복구를 위해,
			이중 검은색 노드 (이동 대상 노드)의 형제 노드 기준 우회전

			- 이중 검은색 노드 (이동 대상 노드)의 형제 노드 (회전 발생 전)의 부모 노드 기준 왼쪽 경로의 검은 노드의 수 변동없음
			- 이중 검은색 노드 (이동 대상 노드)의 형제 노드 (회전 발생 전)의 부모 노드 기준 오른쪽 경로의 검은 노드의 수 증가에 따른 복구
			...
			=> 아직 존재하는 이중 검은색 노드를 이동 대상 노드로 간주하여 이에 대해 판별 및 후속 처리 수행
			: 중복 최적화를 위해 문제의 형태가 -BB BR로 변동되었으므로, 해당 처리 루틴에서 수행 할 것
...
//ok
		...
		(이중 검은색 노드 (이동 대상 노드)의 부모 노드의 오른쪽 자식 == 이중 검은색 노드 (이동 대상 노드))
			...
			=> DEF1)을 위반하는 이중 검은색 노드 (이동 대상 노드)의 한 검은색을 없애기 위해, 
			이중 검은색 노드 (이동 대상 노드)의 형제 노드의 빨간색 노드로 자신의 한 검은색을 전달 할 경우,

			- 이중 검은색 노드 (이동 대상 노드)의 형제 노드 기준 오른쪽 경로의 검은 노드의 수 증가
			- 이중 검은색 노드 (이동 대상 노드)의 부모 노드 기준 오른쪽 경로의 검은 노드의 수 감소
			...
			=> 이중 검은색 노드 (이동 대상 노드)의 형제 노드 기준 오른쪽 경로의 검은 노드의 수 감소를 통한 복구를 위해, 
			먼저, 이중 검은색 노드 (이동 대상 노드)의 형제 노드 기준 좌회전
				
			- 이중 검은색 노드 (이동 대상 노드)의 형제 노드 (회전 발생 전)의 부모 노드 기준 오른쪽 경로의 검은 노드의 수 감소에 따른 복구
			- 이중 검은색 노드 (이동 대상 노드)의 부모 노드 기준 왼쪽 경로의 검은 노드의 수 증가
			...
			=> 이중 검은색 노드 (이동 대상 노드)의 부모 노드 기준 왼쪽 경로의 검은 노드의 수 감소에 따른 복구 및 
			이중 검은색 노드 (이동 대상 노드)의 부모 노드 기준 오른쪽 경로의 검은 노드의 수 증가에 따른 복구를 위해, 
			이중 검은색 노드 (이동 대상 노드)의 부모 노드 기준 우회전
				
			- 이중 검은색 노드 (이동 대상 노드)의 형제 노드 (회전 발생 전)의 부모 노드 기준 왼쪽 경로의 검은 노드의 수 감소에 따른 복구
			- 이중 검은색 노드 (이동 대상 노드)의 형제 노드 (회전 발생 전)의 부모 노드 기준 오른쪽 경로의 검은 노드의 수 증가에 따른 복구
...
//ok
		...
		(이중 검은색 노드 (이동 대상 노드)의 부모 노드의 오른쪽 자식 == 이중 검은색 노드 (이동 대상 노드))
			...
			=> 중복 최적화를 위해 문제의 형태를 -BB BR로 바꿔야 함
			이중 검은색 노드 (이동 대상 노드)의 형제 노드의 오른쪽 빨간색 노드로 자신의 한 검은색을 전달 할 경우,
