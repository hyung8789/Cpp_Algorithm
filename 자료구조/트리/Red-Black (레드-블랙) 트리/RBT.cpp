...
void RBT_RemoveNode(NODE** srcRootNode, const DATA_TYPE& targetData, bool deallocateAfterRemove)
{
	/***
		...
		! 회전 및 색 변경을 수행하는 목적 및 판별 과정에서의 '해당 수행 작업이 수행되어야만 하는 근본적인 판별 방법'은 무엇인가?
		...
		MPC	|	MC	|	MSC	|	MSCC	|	P
		R		B		R		-		throw exception (Invalid Red-Black Tree)
										: DEF2) 위반

		R		R		-		-		이동 대상 노드의 색 변경
										: 빨간색 => 검은색, 
										삭제 된 노드의 부모 노드와 이동 대상 노드 간 DEF2) 위반 수정,
										해당 경로의 검은 노드의 수가 삭제 발생 전 원 상태로 복구되었으므로 DEF4) 위반 수정
...
TODO : ex에 따라 수정 할 것
		...
		B		B		R		-		1) 이동 대상 노드를 이중 검은색 노드로 간주
										...
										2) 이중 검은색 노드의 부모 노드의 색과 이중 검은색 노드의 형제 노드의 색 간의 SWAP
										: 검은색 => 빨간색, 빨간색 => 검은색
										...
										4) 이중 검은색 노드에 대한 이중 검은색 노드의 부모로부터의 자식 방향에 따라,
											...
											: 이중 검은색 노드 < 이중 검은색 노드의 부모 노드 < 이중 검은색 노드의 형제 노드 
												...
												: 이 시점에, 이중 검은색 노드의 형제 노드가 속한 하위 트리에서 이중 검은색 노드의 형제 노드가 루트 노드로 변경,
												회전에 의해 이중 검은색 노드의 형제 노드가 검은색 노드로 변경
										...
										5) DEF1) 위반에 대한 후속 처리를 위해 이동 대상 노드의 인접 노드의 색에 따른 재판별 및 처리 수행
		...
		(이중 검은색 노드의 부모 노드의 오른쪽 자식 == 이중 검은색 노드)
			...
			: 이중 검은색 노드의 형제 노드가 존재하는 경로의 검은 노드의 수 변동없음,
			이중 검은색 노드가 존재하는 경로의 검은 노드의 수 감소

							R
				B (sibling)			DB (moveTarget)
			B		

			=> 이중 검은색 노드의 부모 노드를 기준으로 우회전
			: 회전 발생 후, 이중 검은색 노드의 형제 노드 (회전 발생 전)가 존재하는 왼쪽 경로의 검은 노드의 수 변동없음,
			회전 발생 후, 이중 검은색 노드의 형제 노드 (회전 발생 전)가 존재하는 오른쪽 경로의 검은 노드의 수 복구
			...
			(만약 이 시점에 단순히 이중 검은색 노드를 단일 검은색 노드로 변경만 할 경우 DEF4) 위반)
		...
		(이중 검은색 노드의 부모 노드의 오른쪽 자식 == 이중 검은색 노드)
			...
			: 이중 검은색 노드의 형제 노드가 존재하는 경로의 검은 노드의 수 감소
			...
			: 이중 검은색 노드의 형제 노드가 존재하는 경로의 검은 노드의 수 복구
		...
		(이중 검은색 노드의 부모 노드의 오른쪽 자식 == 이중 검은색 노드)
			...
			///////////
			=> 이중 검은색 노드의 형제 노드를 기준으로 이중 검은색 노드의 빨간색인 자식 노드가 해당 하위 트리에서 루트 노드가 되도록 회전
			: 회전 발생 후, 이중 검은색 노드의 형제 노드 (회전 발생 전)의 부모 노드가 존재하는 왼쪽 경로의 검은 노드의 수 감소,
			회전 발생 후, 이중 검은색 노드의 형제 노드 (회전 발생 전)의 부모 노드가 존재하는 오른쪽 경로의 검은 노드의 수 증가
			...
			: 이중 검은색 노드의 형제 노드 (회전 발생 전)의 부모 노드가 존재하는 왼쪽 및 오른쪽 경로의 검은 노드의 수 복구
...
TODO : RB를 BR 케이스로 바꿀것
		...
		(이중 검은색 노드의 부모 노드의 오른쪽 자식 == 이중 검은색 노드)
			...
			=> 이중 검은색 노드의 부모 노드를 기준으로 우회전
			: 회전 발생 후, 이중 검은색 노드의 형제 노드 (회전 발생 전) 기준 왼쪽 경로의 검은 노드의 수 감소,
			회전 발생 후, 이중 검은색 노드의 형제 노드 (회전 발생 전) 기준 오른쪽 경로의 검은 노드의 수 증가,
