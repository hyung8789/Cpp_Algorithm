...
void RBT_RemoveNode(NODE** srcRootNode, const DATA_TYPE& targetData, bool deallocateAfterRemove)
{
	/***
		...
		삭제 대상 노드의 자리를 대체 할 노드 (이하, 이동 대상 노드) 선택 및 삭제 >
		...
		2) 삭제 대상 노드를 삭제 및 이동 대상 노드를 이동

		3) 삭제 된 노드의 색에 따라,

			3-1) 삭제 된 노드의 색 == 빨간색
			: do nothing (삭제 대상 노드가 빨간색이었으므로, 
		...
		1) 이동 대상 노드의 부모 노드의 색 (R : Red, B : Black)
		2) 이동 대상 노드의 색 (R : Red, B : Black)
		3) 이동 대상 노드의 반대쪽 형제 노드의 색 (R : Red, B : Black)
		4) 이동 대상 노드의 형제 노드의 자식 노드의 색 
		(BB : 모든 자식 노드는 검은색, 
		RR : 모든 자식 노드는 빨간색,
		...
		1	2	3	4	5
		R	R	-	-	1) 이동 대상 노드를 검은색으로 변경
						: 삭제 된 노드의 부모 노드와 이동 대상 노드 간 DEF2) 위반 수정,
						해당 경로의 검은 노드의 수가 감소하였으므로 DEF4) 위반 수정

		B	R	-	-	1) 이동 대상 노드를 검은색으로 변경
						: 해당 경로의 검은 노드의 수가 감소하였으므로 DEF4) 위반 수정

		B	B	R	-	1) 이동 대상 노드를 이중 검은색 노드로 간주
						: DEF4) 위반에서 DEF1) 위반으로 변경

						2) 이중 검은색 노드의 부모 노드의 색 변경
						: 검은색 => 빨간색

						3) 이중 검은색 노드의 형제 노드의 색 변경
						: 빨간색 => 검은색
		...
		1) B	-	B	R	-, 이중 검은색 노드의 부모 노드의 오른쪽 자식 == 이중 검은색 노드
