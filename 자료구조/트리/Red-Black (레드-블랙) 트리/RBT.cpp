...
void RBT_RemoveNode(NODE** srcRootNode, const DATA_TYPE& targetData, bool deallocateAfterRemove)
{
	/***
		...
		! 삭제 발생에 따른 회전 및 색 변경을 수행하는 목적 및 판별 과정에서의 '해당 수행 작업이 수행되어야만 하는 근본적인 판별 방법'은 무엇인가?
		: 이는 각 경로 간 검은 노드의 수의 균형을 유지하기 위함이며,
		이하, '해당 수행 작업이 수행되어야만 하는 근본적인 판별 방법'은 회전 및 색 변경 시 각 경로 간 검은 노드의 균형에 따름
		...
		R		R		-		-		이동 대상 노드의 색 변경 (빨간색 => 검은색)
										: 삭제 된 노드의 부모 노드와 이동 대상 노드 간 DEF2) 위반 수정,
										이동 대상 노드가 존재하는 경로의 검은 노드의 수가 복구되었으므로 DEF4) 위반 수정

		B		R		-		-		이동 대상 노드의 색 변경 (빨간색 => 검은색)
										: 이동 대상 노드가 존재하는 경로의 검은 노드의 수가 복구되었으므로 DEF4) 위반 수정

		B		B		R		-		1) 이동 대상 노드를 이중 검은색 노드로 간주
										: DEF4) 위반에서 DEF1) 위반으로 변경

										2) 이중 검은색 노드의 부모 노드의 색과 이중 검은색 노드의 형제 노드의 색 간의 SWAP (검은색 => 빨간색, 빨간색 => 검은색)
										: 색 변경 후, 이중 검은색 노드의 형제 노드가 존재하는 경로의 검은 노드의 수 변동없음,
										색 변경 후, 이중 검은색 노드가 존재하는 경로의 검은 노드의 수 감소

										4) 이중 검은색 노드에 대한 이중 검은색 노드의 부모로부터의 자식 방향에 따라,

											4-1) 이중 검은색 노드의 부모 노드의 왼쪽 자식 == 이중 검은색 노드
											: 이중 검은색 노드 < 이중 검은색 노드의 부모 노드 < 이중 검은색 노드의 형제 노드 
							
												4-1-1) 이중 검은색 노드의 부모 노드를 기준으로 좌회전
												: 회전 발생 후, 이중 검은색 노드의 형제 노드 (회전 발생 전)의 오른쪽 경로의 검은 노드의 수 변동없음,
												회전 발생 후, 이중 검은색 노드의 형제 노드 (회전 발생 전)의 왼쪽 경로의 검은 노드의 수 복구

											4-2) 이중 검은색 노드의 부모 노드의 오른쪽 자식 == 이중 검은색 노드
											: 이중 검은색 노드의 형제 노드 < 이중 검은색 노드의 부모 노드 < 이중 검은색 노드

												4-2-1) 이중 검은색 노드의 부모 노드를 기준으로 우회전
												: 회전 발생 후, 이중 검은색 노드의 형제 노드 (회전 발생 전)의 왼쪽 경로의 검은 노드의 수 변동없음,
												회전 발생 후, 이중 검은색 노드의 형제 노드 (회전 발생 전)의 오른쪽 경로의 검은 노드의 수 복구

										5) 회전에 의해 이중 검은색 노드의 형제 노드가 검은색 노드로 변경되었으며,
										DEF1) 위반에 대한 후속 처리를 위해 이동 대상 노드의 인접 노드의 색에 따른 재판별 및 처리 수행
										: 이동 대상 노드의 상위 부모 노드가 존재하지 않을 시 (이동 대상 노드가 루트 노드일 시) 처리 종료
...
///이하 수정
		...
		-		B		B		BB		1) 이동 대상 노드를 이중 검은색 노드로 간주
										...
										2) 이중 검은색 노드의 형제 노드의 색 변경
										: 검은색 => 빨간색, 
										이 시점에, 해당 경로의 검은 노드의 수 감소
					
										3) 이중 검은색 노드의 부모 노드의 색에 따라,

											3-1) 이중 검은색 노드의 부모 노드의 색 == 빨간색
							
												3-1-1) 이중 검은색 노드의 한 검은색을 부모 노드로 전달
												: 빨간색 => 검은색, 
												이 시점에, 해당 경로의 검은 노드의 수 다시 복구
								
												3-1-2) 처리 종료
												: 이중 검은색이 단일 검은색으로 변경되었으므로 DEF1) 위반 수정,
												전체 트리의 최상위 루트 노드 기준 양 측 경로 간 DEF4) 위반 사항 존재하지 않음

											3-2) 이중 검은색 노드의 부모 노드의 색 == 검은색

												3-2-1) 이중 검은색 노드의 한 검은색을 부모 노드로 전달
												: 검은색 => 이중 검은색, 
												이 시점에, 해당 경로의 검은 노드의 수 다시 복구

												3-2-2) DEF1) 위반에 대한 후속 처리를 위해 이동 대상 노드의 인접 노드의 색에 따른 재판별 및 처리 수행
												: 이동 대상 노드의 상위 부모 노드가 존재하지 않을 시 (이동 대상 노드가 루트 노드일 시) 처리 종료
	...
	//더 상세히 수정
		1) B	B	R	-
		(이중 검은색 노드의 부모 노드의 오른쪽 자식 == 이중 검은색 노드)

							B
				R (sibling)			DB (moveTarget)
			B				

			=> 이중 검은색 노드의 부모 노드의 색과 이중 검은색 노드의 형제 노드의 색 간의 SWAP (검은색 => 빨간색, 빨간색 => 검은색)
			: 색 변경 후, 이중 검은색 노드의 형제 노드가 존재하는 경로의 검은 노드의 수 변동없음,
			색 변경 후, 이중 검은색 노드가 존재하는 경로의 검은 노드의 수 감소
			...
			=> 이중 검은색 노드가 존재하는 경로의 검은 노드의 수를 증가시키기 위해 이중 검은색 노드의 부모 노드를 기준으로 우회전
			: 회전 발생 후, 이중 검은색 노드의 형제 노드 (회전 발생 전)의 왼쪽 경로의 검은 노드의 수 변동없음,
			회전 발생 후, 이중 검은색 노드의 형제 노드 (회전 발생 전)의 오른쪽 경로의 검은 노드의 수 증가에 따른 복구

							B (oldSibling)
					B				R
										DB (moveTarget)
			
			=> 이중 검은색 노드의 형제 노드가 검은색 노드로 변경되었으며,
			DEF1) 위반에 대한 후속 처리를 위해 이동 대상 노드의 인접 노드의 색에 따른 재판별 및 처리 수행
...
//더 상세히 수정
		...
		(이중 검은색 노드의 부모 노드의 오른쪽 자식 == 이중 검은색 노드)
			...
			=> 이중 검은색 노드의 형제 노드의 색 변경 (검은색 => 빨간색) 
			: 색 변경 후, 이중 검은색 노드의 형제 노드가 존재하는 경로의 검은 노드의 수 감소
		
							B (or R)
				R (sibling)			DB (moveTarget)
			B		B

			=> 이중 검은색 노드의 한 검은색을 부모 노드로 전달 (검은색 => 이중 검은색 or 빨간색 => 검은색)
			: 색 전달 후, 이중 검은색 노드의 형제 노드가 존재하는 경로의 검은 노드의 수 증가에 따른 복구
		...
		(이중 검은색 노드의 부모 노드의 오른쪽 자식 == 이중 검은색 노드)
			...
			=> 이동 대상 노드의 부모 노드의 색에 따라,

				1) 이동 대상 노드의 부모 노드의 색 == 빨간색
				2) 이동 대상 노드의 부모 노드의 색 == 검은색
...
//ok
		...
		(이중 검은색 노드의 부모 노드의 오른쪽 자식 == 이중 검은색 노드)
			...
			=> 이동 대상 노드의 부모 노드의 색에 따라,
				...
				=> DEF1)을 위반하는 이중 검은색 노드의 한 검은색을 없애기 위해, 
				이중 검은색 노드의 부모 노드로 자신의 한 검은색을 전달 할 경우,
				이중 검은색 노드의 부모 노드 기준 왼쪽 경로의 검은 노드의 수가 증가하며,
				이중 검은색 노드의 부모 노드 기준 왼쪽 경로의 검은 노드의 수 감소를 통한 복구를 위해, 
				이중 검은색 노드의 형제 노드 기준 좌회전
				...
				=> 이동 대상 노드의 부모 노드 기준 우회전
				: 회전 발생 후, 이중 검은색 노드의 형제 노드 (회전 발생 전)의 부모 노드 기준 왼쪽 경로의 검은 노드의 수 감소에 따른 복구,
				회전 발생 후, 이중 검은색 노드의 형제 노드 (회전 발생 전)의 부모 노드 기준 오른쪽 경로의 검은 노드의 수 변동없음

								R
					B (oldSibling)		B
				B							B (moveTarget)
						
				=> DEF1) 및 DEF4)의 위반사항 수정 완료
				...
				=> DEF1)을 위반하는 이중 검은색 노드의 한 검은색을 없애기 위해, 
				이중 검은색 노드의 형제 노드의 빨간색 노드로 자신의 한 검은색을 전달 할 경우,
				이중 검은색 노드의 형제 노드 기준 오른쪽 경로의 검은 노드의 수가 증가하며,
				이중 검은색 노드의 형제 노드 기준 오른쪽 경로의 검은 노드의 수 감소를 통한 복구를 위해, 
				이중 검은색 노드의 형제 노드 기준 좌회전
				: 회전 발생 후, 이중 검은색 노드의 형제 노드 (회전 발생 전)의 부모 노드 기준 오른쪽 경로의 검은 노드의 수 감소에 따른 복구,
				회전 발생 후, 이동 대상 노드의 부모 노드 기준 왼쪽 경로의 검은 노드의 수 증가
				...
				=> 이동 대상 노드의 부모 노드 기준 왼쪽 경로의 검은 노드의 수 감소에 따른 복구를 위해, 이중 검은색 노드의 부모 노드 기준 우회전
				: 회전 발생 후, 이중 검은색 노드의 형제 노드 (회전 발생 전)의 부모 노드 기준 왼쪽 경로의 검은 노드의 수 감소에 따른 복구,
				회전 발생 후, 이중 검은색 노드의 형제 노드 (회전 발생 전)의 부모 노드 기준 오른쪽 경로의 검은 노드의 수 변동없음
			...
			=> DEF1)을 위반하는 이중 검은색 노드의 한 검은색을 없애기 위해, 이중 검은색 노드가 존재하는 경로의 검은 노드의 수를 증가시켜야 하지만,
			이를 위해 먼저 이중 검은색 노드의 부모 노드를 기준으로 우회전할 경우, 
			이중 검은색 노드의 형제 노드 (회전 발생 전) 기준 왼쪽 경로의 검은 노드의 수 변동없거나 감소하므로,
			이중 검은색 노드의 형제 노드 (회전 발생 전) 기준 왼쪽 경로의 검은 노드의 수를 유지하기 위해, 먼저 이중 검은색 노드의 형제 노드를 기준으로 좌회전
			: 회전 발생 후, 이중 검은색 노드의 형제 노드 (회전 발생 전)의 부모 노드 기준 오른쪽 경로의 검은 노드의 수 감소
			...
			=> 이중 검은색 노드의 형제 노드 (회전 발생 전)의 부모 노드 기준 오른쪽 경로의 검은 노드의 수 감소에 따른
			이중 검은색 노드의 형제 노드 (회전 발생 전)의 부모 노드 기준 오른쪽 경로의 검은 노드의 수 복구를 위해
			이중 검은색 노드의 형제 노드 (회전 발생 전)와 이중 검은색 노드의 형제 노드 (회전 발생 전)의 부모 노드의 색 간의 SWAP
			: 색 변경 발생 후, 이중 검은색 노드의 형제 노드 (회전 발생 전)의 부모 노드 기준 오른쪽 경로의 검은 노드의 수 증가에 따른 복구
			...
			=> 마지막으로, DEF1)을 위반하는 이중 검은색 노드의 한 검은색을 없애기 위해 
			이중 검은색 노드가 존재하는 경로의 검은 노드의 수를 증가시켜야 하므로 이중 검은색 노드의 부모 노드를 기준으로 우회전
			: 회전 발생 후, 이중 검은색 노드의 형제 노드 (회전 발생 전)의 부모 노드 기준 왼쪽 경로의 검은 노드의 수 변동없거나 감소,
			회전 발생 후, 이중 검은색 노드의 형제 노드 (회전 발생 전)의 부모 노드 기준 오른쪽 경로의 검은 노드의 수 변동없거나 증가
...
//수정
		...
		(이중 검은색 노드의 부모 노드의 오른쪽 자식 == 이중 검은색 노드)
			...
			=> DEF1)을 위반하는 이중 검은색 노드의 한 검은색을 없애기 위해
			이중 검은색 노드가 존재하는 경로의 검은 노드의 수를 증가시켜야 하므로, 이중 검은색 노드의 부모 노드를 기준으로 우회전
			: 회전 발생 후, 이중 검은색 노드의 형제 노드 (회전 발생 전) 기준 왼쪽 경로의 검은 노드의 수 변동없거나 감소,
			회전 발생 후, 이중 검은색 노드의 형제 노드 (회전 발생 전) 기준 오른쪽 경로의 검은 노드의 수 변동없거나 증가,
			...
			=> 이동 대상 노드의 부모 노드의 색에 따라,
				...
				1) 이동 대상 노드의 부모 노드의 색 == 빨간색
				: 회전 발생 후, 이중 검은색 노드의 형제 노드 (회전 발생 전) 기준 왼쪽 경로의 검은 노드의 수가 변동없으며,
				이중 검은색 노드의 이중 검은색 노드의 형제 노드 (회전 발생 전) 기준 오른쪽 경로의 검은 노드의 수가 증가하였음을 의미
				이중 검은색 노드의 한 검은색을 자신의 부모 노드로 전달하면, ???????
			

				이중 검은색 노드의 한 검은색을 이동 대상 노드의 부모 노드의 색으로 전달 (빨간색 => 검은색)
				...
				2) 이동 대상 노드의 부모 노드의 색 == 검은색
				: 회전 발생 후, 이중 검은색 노드의 형제 노드 (회전 발생 전) 기준 왼쪽 경로의 검은 노드의 수가 감소하였으며,
				이중 검은색 노드의 형제 노드 (회전 발생 전) 기준 오른쪽 경로의 검은 노드의 수는 증가하였음을 의미
				이중 검은색 노드의 한 검은색을 이중 검은색 노드의 형제 노드 (회전 발생 전)의 왼쪽 자식 노드로 
				형제 노드 (회전 발생 전)의 왼쪽 자식 노드의 색을 이중 검은색 노드의 한 검은색으로 변경 (빨간색 => 검은색)
